...
function objToTSNode(factory, obj) {
    var props = Object.entries(obj).map(function (_a) {
        var k = _a[0], v = _a[1];
        return factory.createPropertyAssignment(k, primitiveToTSNode(factory, v) ||
            (Array.isArray(v)
                ? factory.createArrayLiteralExpression(v.map(function (n) { return objToTSNode(factory, n); }))
                : typeof v === 'object'
...
function extractMessageFromJsxComponent(ts, factory, node, opts, sf, ctx) {
    ...
    var visitor = function (node) {
        if (ts.isJsxAttributes(node)) {
            return factory.createJsxAttributes(generateNewProperties(ts, factory, node, {
    ...
    var newProps = tslib_1.__spreadArrays([
        ...
        ? [
            factory.createPropertyAssignment('defaultMessage', ast
                ? messageASTToTSNode(factory, intl_messageformat_parser_1.parse(msg.defaultMessage))
    ...
    var newProps = tslib_1.__spreadArrays([
        ...
        ? [
            factory.createJsxAttribute(factory.createIdentifier('defaultMessage'), ast
                ? factory.createJsxExpression(undefined, messageASTToTSNode(factory, intl_messageformat_parser_1.parse(msg.defaultMessage)))
                : factory.createStringLiteral(msg.defaultMessage)),
        ]
        : []));
    for (var _i = 0, _a = node.properties; _i < _a.length; _i++) {
...
function extractMessagesFromCallExpression(ts, factory, node, opts, sf) {
    ...
    if (isMultipleMessageDecl(ts, node)) {
        ...
        if (descriptorsObj) {
            ...
            return factory.createCallExpression(node.expression, node.typeArguments, factory.createNodeArray(tslib_1.__spreadArrays([clonedDescriptorsObj], restArgs)));
        }
    }
        ...
        if (ts.isObjectLiteralExpression(descriptorsObj)) {
            ...
            return factory.createCallExpression(node.expression, node.typeArguments, factory.createNodeArray(tslib_1.__spreadArrays([
